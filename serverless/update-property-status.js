...
module.exports = async (req, res) => {
  ...
  const ip = getIP(req);
  const rl = rateLimit("_updateStatusMap", ip, 20, 60 * 60 * 1000);
  if (rl.limited) return res.status(429).json({ error: "Too many requests" });

  const { recordId, status, token, email } = req.body || {};
  if (!recordId || !status) return res.status(400).json({ error: "Missing required fields: recordId and status" });
  ...
  try {
    ...
    if (contactEmail.toLowerCase() !== email.toLowerCase()) {
      console.warn(`[STATUS BLOCKED] Email mismatch for property ${recordId} from IP ${ip}`);
    ...
    if (!response.ok) return res.status(response.status).json({ error: "Failed to update property status" });

    const data = await response.json();
    console.log(`[STATUS UPDATE] Property ${recordId} -> ${status} by ${email.substring(0,3)}***@*** from IP ${ip}`);
    return res.status(200).json({ success: true, message: "Property status updated successfully", record: data });
  } catch (error) {
